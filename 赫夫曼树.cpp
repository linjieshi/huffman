	...
	*huffmanTree = (HuffmanTree)malloc((m + 1) * sizeof(Node));
	...
	for (i = n + 1; i <= m; i++)
	{
		...
		(*huffmanTree)[i].weight = (*huffmanTree)[s1].weight +
			(*huffmanTree)[s2].weight;
		printf("%d (%d, %d)\n", (*huffmanTree)[i].weight,
			(*huffmanTree)[s1].weight, (*huffmanTree)[s2].weight);
	}
	printf("\n");
}
...
void creatHuffmanCode(HuffmanTree *huffmanTree, HuffmanCode *huffmanCode, int n)
{
	...
	for (i = 1; i <= n; i++)
	{
		...
		start = n - 1;
		//从叶子到根结点求编码
		for (c = i, p = (*huffmanTree)[i].parent; p != 0; c = p, p =
			(*huffmanTree)[p].parent)
		{
...
int main(void)
{
	...
	printf("input the %d element's weight:\n", n);
	for (i = 1; i <= n; i++)
	{
		printf("%d: ", i);
		fflush(stdin);
